<?php include("includes/connection.php");
	...
	else if(isset($_GET['home']))
	{
		...
		else if(isset($_GET['cource_category_list']))
 	{
 		$jsonObj= array();
		...
		while($data = mysqli_fetch_assoc($sql))
		{
			...
			$row['image']          = "http://padhakuji.com/video/images/".$data['category_image'];
		...
		else if(isset($_GET['cource_list_by_catid']))
 	{
 		$jsonObj= array();
		
		$cat_order=API_CAT_ORDER_BY;
       $cat_id = $_GET['cource_list_by_catid'];

		$query="SELECT * FROM cource c join tbl_category tc on c.cat_id  = tc.cid where c.cat_id = '$cat_id'";
		$sql = mysqli_query($mysqli,$query)or die(mysql_error());
		...
		while($data = mysqli_fetch_assoc($sql))
		{
            ...
            $row['category_name']     = $data['category_name'];
			$row['cource_id']     = $data['cource_id'];
			$row['title']         = $data['title'];
			$row['price']         = $data['price'];
			$row['duration']      = $data['duration'];
			$row['description']   = $data['description'];
			$row['icon']          = "http://padhakuji.com/video/images/".$data['icon'];
	...
	else if(isset($_GET['cource_list_by_courceid']))
 	{
 		$jsonObj= array();
		
		$cat_order=API_CAT_ORDER_BY;
        $cource_id = $_GET['cource_list_by_courceid'];

		$query="SELECT * FROM cource c join tbl_category tc on c.cat_id  = tc.cid where c.cource_id = '$cource_id'";
		$sql = mysqli_query($mysqli,$query)or die(mysql_error());
		while($data = mysqli_fetch_assoc($sql))
		{
			$row['cource_id']     = $data['cource_id'];
			...
			$row['price']         = $data['price'];
			$row['duration']      = $data['duration'];
			$row['description']   = preg_replace('/[^A-Za-z0-9\-]/', '',strip_tags($data['description']));
			$row['icon']          = "http://padhakuji.com/video/images/".$data['icon'];
			$row['thumbnail']     = "http://padhakuji.com/video/images/".$data['icon'];
			...
			$cat_id               = $data['cat_id'];
	        		$jsonObj2     = array();
	        		$query2       = "SELECT * FROM tbl_video where cat_id = '$cat_id'"; 
	             	$sql2         = mysqli_query($mysqli,$query2)or die(mysql_error());
	            	while($data2  = mysqli_fetch_assoc($sql2))
		            {
		              $row2['video_id']        = $data2['id'];
		              $row2['video_title']     = $data2['video_title'];
		              $row2['video_url']       = $data2['video_url'];
		              $row2['video_thumbnail'] = "http://padhakuji.com/video/images/".$data2['video_thumbnail'];
		              array_push($jsonObj2,$row2);  
		            }
		
            $row['related_video']     = $jsonObj2;
			array_push($jsonObj,$row);
	...
	else if(isset($_GET['login']))
 	{
		...
		die();
 		...
 		else if(isset($_GET['free_video']))
 	{
 		$jsonObj= array();
		$cat_order=API_CAT_ORDER_BY;
		$query="select * from  tbl_video where featured_video = '0'";
		$sql = mysqli_query($mysqli,$query)or die(mysql_error());
		while($data = mysqli_fetch_assoc($sql))
		{
            $row['video_id']         = $data['id'];
            $row['title']            = $data['video_title'];
            $row['video_url']        = $data['video_url'];
            $row['video_description']= $data['video_description'];
            $row['video_thumbnail']  = "http://padhakuji.com/video/images/".$data['video_thumbnail'];
            $row['cat_id']           = $data['cat_id'];
            $cat_id                  = $data['cat_id'];
            $vid_id                  = $data['id'];
                    $jsonObj2        = array();
                 	$query2          = "select * from  tbl_video where cat_id = '$cat_id' AND featured_video = '0' ";
	             	$sql2            = mysqli_query($mysqli,$query2)or die(mysql_error());
            		while($data2     = mysqli_fetch_assoc($sql2))
            		{
                		$row2['video_id']         = $data2['id'];
                        $row2['title']            = $data2['video_title'];
                        $row2['video_url']        = $data2['video_url'];
                        $row2['video_description']= $data2['video_description'];
                        $row2['video_thumbnail']  = "http://padhakuji.com/video/images/".$data['video_thumbnail'];
 	...
 	else if(isset($_GET['premium_video']))
 	{
 		$jsonObj    = array();
		$cat_order  = API_CAT_ORDER_BY;
		$query      = "select * from  tbl_video where featured_video = '1'";
		$sql        = mysqli_query($mysqli,$query)or die(mysql_error());
		while($data = mysqli_fetch_assoc($sql))
		{
            $row['video_id']         = $data['id'];
            ...
            $vid_id                  = $data['id'];
            $jsonObj2 = array();
            $query2   ="select * from  tbl_video where cat_id = '$cat_id' AND featured_video = '1' ";
            $sql2     = mysqli_query($mysqli,$query2)or die(mysql_error());
            while($data2 = mysqli_fetch_assoc($sql2))
            {
                $row2['video_id']     = $data2['id'];
	...
	else if(isset($_GET['recent_event']))
 	{ //echo "ok";die;
 		$jsonObj= array();
		 $cat_order=API_CAT_ORDER_BY;
		 $url="http://padhakuji.com/video/images/";
		$query="select event.*,CONCAT('$url',main_image)  as image_url from  event order by event_id DESC LIMIT 10";
		$sql = mysqli_query($mysqli,$query)or die(mysql_error());
		...
		die();
 		...
 		else if(isset($_GET['detail_event']))
 	{ //echo "ok";die;
 	  	 $jsonObj= array();
 		 $event_id=$_GET['detail_event'];
		 $cat_order=API_CAT_ORDER_BY;
		 $url="http://padhakuji.com/video/images/";
		$query="select *,CONCAT('$url',main_image)  as image_url from  `event` e join tbl_category c on e.category = c.cid where e.event_id='$event_id'";
		$sql = mysqli_query($mysqli,$query)or die(mysql_error());
		while($row = mysqli_fetch_assoc($sql))
		{   
		    $jsonObj2= array();
		    $query2="select *,CONCAT('$url',img)  as image_url from  `event_gallary` where event_id='$event_id'";
    		$sql2 = mysqli_query($mysqli,$query2)or die(mysql_error());
	...
	else if(isset($_GET['popular_event']))
 	{ //echo "ok";die;
 		$jsonObj= array();
		 $cat_order=API_CAT_ORDER_BY;
		 $url="http://padhakuji.com/video/images/";
		$query="select event.*,CONCAT('$url',main_image)  as image_url from  event  where status = 0 order by event_id DESC ";
		$sql = mysqli_query($mysqli,$query)or die(mysql_error());
		while($row = mysqli_fetch_assoc($sql))
		{
	...
	else if(isset($_GET['video_id']))
	{
	 ...
	 else if(isset($_GET['subject_wise_videos']) && isset($_GET['user_id']))
 	{
 	  $query=mysqli_query($mysqli,"select * from user where user_id='".$_GET['user_id']."'");
 	  while($row=mysqli_fetch_assoc($query))
 	  {
		...
		$cat_order=API_CAT_ORDER_BY;
		$url = 'http://padhakuji.com/video/images/';
		$query="SELECT tbl_video.*,concat('$url',video_thumbnail) as raal_thumbnail  FROM tbl_video where subject_id = '".$_GET['subject_wise_videos']."' 
		         ";
		$sql = mysqli_query($mysqli,$query)or die(mysql_error());
		while($data = mysqli_fetch_assoc($sql))
		{
 	...
 	else if(isset($_GET['subject_wise_videos1']) && isset($_GET['videosid']))
 	{
 	    $jsonObj= array();
		$cat_order=API_CAT_ORDER_BY;
		$url = 'http://padhakuji.com/video/images/';
		$query="SELECT tbl_video.*,concat('$url',video_thumbnail) as raal_thumbnail  FROM tbl_video where subject_id = '".$_GET['subject_wise_videos1']."' 
		          AND tbl_video.id!='".$_GET['videosid']."'";
		$sql = mysqli_query($mysqli,$query)or die(mysql_error());
		...
		while($data = mysqli_fetch_assoc($sql))
		{
			//$row['id']             = $data['cid'];
			//$row['name']           = $data['category_name'];
			//$row['image']          = "http://ksbmresearch.com/video/images/".$data['category_image'];
